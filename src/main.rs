...
mod tests {
    ...
    fn test_custom_values() {
        let matches = create_test_app()
            .try_get_matches_from(vec!["tcping", "google.com", "443", "-n", "5", "-t", "10", "-i", "1000"])
            .unwrap();
    ...
    fn test_long_argument_names() {
        let matches = create_test_app()
            .try_get_matches_from(vec!["tcping", "google.com", "443", "--num", "20", "--timeout", "5", "--interval", "200"])
            .unwrap();
    ...
    fn test_parse_numeric_values() {
        let matches = create_test_app()
            .try_get_matches_from(vec!["tcping", "localhost", "22", "-n", "15", "-t", "2", "-i", "750"])
            .unwrap();
    ...
    fn test_zero_values() {
        let matches = create_test_app()
            .try_get_matches_from(vec!["tcping", "localhost", "22", "-n", "0", "-t", "0", "-i", "0"])
            .unwrap();
        
        // Test parsing zero values
        let num_pings = matches.get_one::<String>("n").unwrap().parse::<u32>().unwrap();
        let timeout = matches.get_one::<String>("t").unwrap().parse::<u64>().unwrap();
        let interval = matches.get_one::<String>("i").unwrap().parse::<u64>().unwrap();
